# Production environment configuration for AWS GPU Spot Monitor

terraform {
  required_version = ">= 1.0.0"
  
  backend "s3" {
    bucket         = "aws-gpu-spot-monitor-terraform-state-prod"
    key            = "terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "aws-gpu-spot-monitor-terraform-state-lock-prod"
    encrypt        = true
  }
}
