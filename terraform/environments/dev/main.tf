# Dev environment configuration for AWS GPU Spot Monitor

terraform {
  required_version = ">= 1.0.0"
  
  backend "s3" {
    bucket         = "aws-gpu-spot-monitor-terraform-state-dev"
    key            = "terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "aws-gpu-spot-monitor-terraform-state-lock-dev"
    encrypt        = true
  }
}
